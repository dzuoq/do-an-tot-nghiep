...
const Payment: React.FC<IProps> = (props) => {
  ...
  const handleVNPayPayment = async () => {
    try {
      ...
      // Kiểm tra nếu số lượng sản phẩm trong giỏ lớn hơn stock
      const outOfStockProducts = cart.filter(
        (item: { count: number; stock: number }) => item.count > item.stock
      );

      if (outOfStockProducts.length > 0) {
        // Lấy tên các sản phẩm không đủ tồn kho
        const productNames = outOfStockProducts
          .map((item: { productName: any }) => item.productName)
          .join(", ");
        toast.error(`Sản phẩm không đủ tồn kho: ${productNames}`);
        return; // Ngăn không tiếp tục thực hiện thanh toán
      }
  ...
  const handlePaymentSubmit = async (method: string) => {
    try {
      ...
      const response = await createOrder(
        ...
        method,
        orderCode,
        props.selectedAddress.addressBookId,
        discount
      );
  ...
  const handleApplyVoucher = async () => {
    try {
      ...
      if (response?.payload) {
        const { discount, isUsed, voucherId } = response.payload; // Lưu voucherId
                            ...
                            createOrder={(data, actions) => {
                              ...
                              if (outOfStockProducts.length > 0) {
                                ...
                                const productNames = outOfStockProducts
                                  ...
                                  .join(", ");
                                toast.error(
                                  `Sản phẩm không đủ tồn kho: ${productNames}`
                                );
                                return Promise.reject(
                                  new Error("Sản phẩm không đủ tồn kho")
                                );
                              }

                              // Tạo đơn hàng PayPal nếu tồn kho hợp lệ
